...
fp = Image.open("test1.jpeg")
pixels = fp.load()
w, h = fp.size
image_data = [[[0 for k in range(3)] for j in range(h)] for i in range(w)]
for i in range(w):
    for j in range(h):
        image_data[i][j][0] = pixels[i, j][0]
        image_data[i][j][1] = pixels[i, j][1]
        image_data[i][j][2] = pixels[i, j][2]
